...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        words, cache_words = word_embedding_forward(captions_in, W_embed)
        if self.cell_type == "rnn":
            h, cache_h = rnn_forward(words, h0, Wx, Wh, b)
        elif (self.cell_type == 'lstm'):
            h, cache_h = lstm_forward(words, h0, Wx, Wh, b)
        ...
        if self.cell_type == 'rnn':
            dx, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dout, cache_h)
        elif self.cell_type == 'lstm':
            dx, dh0, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(dout, cache_h)
        grads['W_embed'] = word_embedding_backward(dx, cache_words)
    ...
    def sample(self, features, max_length=30):
        ...
        word = np.ones(N, dtype=np.int) * self._start
        if self.cell_type == 'lstm': c = np.zeros(h.shape)
